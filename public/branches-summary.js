document.addEventListener('DOMContentLoaded', async () => {
  ...
  function showBranchSelector() {
    ...
    branchesGrid.innerHTML = availableBranches.map(branch => `
  ...
  function displayBranchSummary(branchId, data) {
    ...
    document.getElementById('branchTitle').textContent = branch ? branch.name : 'Sucursal';
    document.getElementById('totalControls').textContent = data.stats.total_controls;
    document.getElementById('completedControls').textContent = data.stats.completed_controls;
    document.getElementById('draftControls').textContent = data.stats.draft_controls;
    
    // Actualizar contador de items
    document.getElementById('itemsCount').textContent = `${data.controls.length} controles`;
  ...
  function displayControlsTable(controls) {
    ...
    if (controls.length === 0) {
      tbody.innerHTML = '<tr><td colspan="10" class="text-center">No hay controles registrados para esta sucursal</td></tr>';
      return;
    }
    ...
    controls.forEach(control => {
      ...
      row.innerHTML = `
        ...
        <td>${control.control_date ? new Date(control.control_date).toLocaleDateString() : 'N/A'}</td>
        <td>
          <div style="display: flex; gap: 5px;">
            <a href="/stock.html?branch=${control.branch_id}" class="btn btn-primary btn-sm">
              Ver Detalle
            </a>
  ...
  async function deleteControl(controlId) {
    if (!confirm('¿Estás seguro de eliminar este control? Esta acción eliminará el control y todos sus productos asociados. No se puede deshacer.')) {
      return;
    }
  ...
  function setupEventListeners() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (branchesBtn && branchesSubmenu && !branchesBtn.contains(e.target) && !branchesSubmenu.contains(e.target)) {
        branchesSubmenu.classList.remove('show');
      }
      if (stockBtn && stockSubmenu && !stockBtn.contains(e.target) && !stockSubmenu.contains(e.target)) {
