...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadCatalogs() {
    try {
      ...
      // Cargar productos
      const productsResponse = await fetch('/api/stock/catalogs/products?limit=1000', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  async function loadCurrentControl() {
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`/api/stock/monthly-control/current?branch_id=${currentBranchId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  function updateControlDisplay() {
    const branchDisplayName = currentUser.role === 'employee' 
    ...
    elements.controlStatus.textContent = currentControl.status === 'draft' ? 'Borrador' : 'Completado';
  }
  ...
  function updateStatsDisplay(stats) {
    ...
    document.getElementById('needOrderItems').textContent = stats.need_order || 0;
    ...
    document.getElementById('avgCompliance').textContent = `${stats.avg_compliance || 0}%`;
  }
  ...
  function showCreateControlOption(period) {
    ...
    elements.branchName.textContent = getBranchNameById(currentBranchId) || 'Sucursal';
  ...
  function showBranchSelector() {
    ...
    let selectorHTML = '<div class="branch-selector"><h3>Seleccionar Sucursal</h3>';
  ...
  function setupProductAutocomplete() {
    ...
    function showProductSuggestions(query) {
      const matches = catalogData.products.filter(product => 
      ...
      suggestionsDiv.innerHTML = matches.map(product => `
        <div class="suggestion-item" data-product-id="${product.id}">
          ...
          <small>${product.product_code} ${product.description ? '- ' + product.description : ''}</small>
        </div>
      `).join('');
  ...
  function populateProductStatusSelect() {
    ...
    select.innerHTML = '<option value="1">Activo</option><option value="2">Inactivo</option><option value="3">Prueba</option>';
  }
  ...
  function populateProductStatusFilter() {
    ...
    catalogData.productStatus.forEach(status => {
      ...
      option.textContent = status.product_status_name || status.name || `Estado ${status.id}`;
      select.appendChild(option);
    });
  }
  ...
  async function completeControl() {
    if (!confirm('¿Estás seguro de completar este control? No podrás editarlo después.')) {
      return;
    }
  ...
  async function loadControlItems(page = 1) {
    ...
    try {
      ...
      const response = await fetch(`/api/stock/items/${currentControl.id}?${params}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  async function updateProduct(itemId, stockRequire, stockCurrent, notes) {
    console.log('updateProduct llamado:', { itemId, stockRequire, stockCurrent, notes });
    try {
      const token = localStorage.getItem('authToken');
  ...
  function displayItems(items) {
    ...
    if (items.length === 0) {
      tbody.innerHTML = '<tr><td colspan="9" class="text-center">No hay productos en este control</td></tr>';
      return;
    }
  ...
  function createItemRow(item) {
    ...
    const stockDifferenceClass = item.stock_difference >= 0 ? 'compliance-positive' : 'compliance-negative';
    const complianceClass = item.stock_compliance >= 80 ? 'compliance-positive' : 'compliance-negative';
    ...
    row.innerHTML = `
      ...
      <td class="editable-status-cell" data-field="product_status_id" data-item-id="${item.id}">
        ${item.product_status_name || 'Activo'}
      </td>
      <td class="editable-cell" data-field="stock_require" data-item-id="${item.id}">${item.stock_require}</td>
      <td class="editable-cell" data-field="stock_current" data-item-id="${item.id}">${item.stock_current}</td>
      <td class="${stockDifferenceClass}">${item.stock_difference > 0 ? '+' : ''}${item.stock_difference}</td>
      <td class="${complianceClass}">${item.stock_compliance}%</td>
      <td>
        <span class="status-badge status-${item.stock_status_name || 'stock_optimo'}">
  ...
  function makeEditable(cell, item) {
    ...
    const save = async () => {
      ...
      const updateData = {
        stock_require: field === 'stock_require' ? newValue : item.stock_require,
        stock_current: field === 'stock_current' ? newValue : item.stock_current
      };

      await updateProduct(itemId, updateData.stock_require, updateData.stock_current, item.notes);
    };
  ...
  function updatePagination(pagination) {
    ...
    prevBtn.addEventListener('click', () => loadControlItems(pagination.page - 1));
    ...
    nextBtn.addEventListener('click', () => loadControlItems(pagination.page + 1));
  ...
  function setupEventListeners() {
    ...
    elements.addProductForm.addEventListener('submit', (e) => {
      ...
      const data = {
        ...
        product_status_id: document.getElementById('productStatusSelect').value || 1,
      ...
      if (!data.product_id || !data.category_id || !data.condition_id || !data.stock_require || !data.stock_current) {
    ...
    document.getElementById('conditionFilter').addEventListener('change', handleFilterChange);
    document.getElementById('productStatusFilter').addEventListener('change', handleFilterChange);
    document.getElementById('stockStatusFilter').addEventListener('change', handleFilterChange);
